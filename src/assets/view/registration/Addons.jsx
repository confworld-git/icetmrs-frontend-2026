...
import { addonsList } from "../../../utils/jouralData";
    ...
    <div className="py-6 px-4 md:px-6">
      ...
      <div className="flex justify-center mb-6">
        <button
          ...
          className="flex items-center gap-2 bg-indigo-800 text-white px-8 py-3 rounded-xl font-semibold text-sm hover:bg-indigo-700 transition-all duration-200 shadow-md shadow-indigo-200 hover:shadow-lg"
        >
          <span>☰</span>
          {expanded ? "Hide" : "Add"} Addons
          {selectedAddons.length > 0 && (
            <span className="bg-white text-indigo-800 text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center ml-1">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${
                    selected
                      ? "border-indigo-800 shadow-lg shadow-indigo-100 scale-[1.02]"
                      : "border-gray-200 bg-white shadow-sm hover:border-indigo-600 hover:shadow-md hover:scale-[1.01]"
                  }
                `}
              >
                {/* Coloured top strip */}
                <div className="bg-indigo-800 px-4 pt-4 pb-5">
                  <div className="flex justify-between items-start">
                    ...
                    >
                      {selected && (
                        <span className="text-indigo-800 text-[10px] font-bold">✓</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {selectedAddons.length > 0 && (
          <div className="mt-5 bg-indigo-50 border border-indigo-200 rounded-xl p-4">
            ...
            <div className="mt-3 pt-3 border-t border-indigo-200 flex justify-between">
              <span className="font-semibold text-sm text-indigo-900">Add-ons Total</span>
              <span className="font-bold text-indigo-900">+${totalAddonsAmount}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {!expanded && selectedAddons.length > 0 && (
        <div className="mt-3 flex items-center justify-between bg-indigo-800 rounded-xl px-5 py-3 shadow-md shadow-indigo-200">
          <div className="flex items-center gap-2">
            <span className="w-5 h-5 bg-white rounded-full flex items-center justify-center">
              <span className="text-indigo-800 text-xs font-bold">{selectedAddons.length}</span>
            </span>
